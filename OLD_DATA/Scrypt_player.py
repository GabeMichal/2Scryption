        ...
        if len(deck) > 0:
            ...
            if len(self.totem) == 2:
                if self.totem[0] == card.tribe:
                    card.sigils2.append(self.totem[1])
                    print(f"Your {card.tribe} gained the {self.totem[1]} sigil.")
            if "Amorphous" in card.sigils:
                card.sigils.remove("Amorphous")
                card.sigils.append(random.choice(sigilList2))
                print(f"This card's sigil is {card.sigils[-1]}")
            if "Amorphous" in card.sigils2:
                card.sigils2.remove("Amorphous")
                card.sigils2.append(random.choice(sigilList2))
                print(f"This card's sigil is {card.sigils2[-1]}")
            if "Finical Hatchling" in card.sigils+card.sigils2:
                ...
                if 0 not in fhpows and 0 not in fhhelt and 0 not in fhtrib:
                    print(f"Your {card.name} has hatched... what a wonderful form.")
